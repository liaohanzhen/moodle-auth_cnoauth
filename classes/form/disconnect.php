...
class disconnect extends \moodleform {
    ...
    protected function definition() {
        ...
        $mform =& $this->_form;
        $mform->addElement('html', \html_writer::tag('h4', get_string('ucp_disconnect_title', 'auth_cnoauth', $opname)));
        $mform->addElement('html', \html_writer::div(get_string('ucp_disconnect_details', 'auth_cnoauth', $opname)));
        ...
        $mform->addGroup($newmethod, 'newmethodar', get_string('errorauthdisconnectnewmethod', 'auth_cnoauth'), [' '], false);
        if (!empty($this->_customdata['prevmethod'])) {
        ...
        if ($manualenabled === true) {
            $mform->addElement('html', \html_writer::div(get_string('errorauthdisconnectifmanual', 'auth_cnoauth')));
