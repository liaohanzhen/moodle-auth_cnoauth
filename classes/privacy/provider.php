...
if (interface_exists('\core_privacy\local\request\core_userlist_provider')) {
    interface auth_cnoauth_userlist extends \core_privacy\local\request\core_userlist_provider {}
} else {
    interface auth_cnoauth_userlist {};
}

class provider implements
    \core_privacy\local\request\plugin\provider,
    \core_privacy\local\metadata\provider,
    auth_cnoauth_userlist {
    ...
    public static function get_contexts_for_userid(int $userid) : contextlist {
        ...
        $sql = "SELECT ctx.id
                  FROM {auth_cnoauth_prevlogin} pv
    ...
    public static function get_users_in_context(\core_privacy\local\request\userlist $userlist) {
        ...
        $sql = "SELECT ctx.instanceid as userid
                  FROM {auth_cnoauth_prevlogin} pl
                  JOIN {context} ctx
        ...
        foreach ($tables as $table => $filterparams) {
            ...
            foreach ($records as $record) {
                writer::with_context($context)->export_data([
                    ...
                    get_string('privacy:metadata:'.$table, 'auth_cnoauth')
                ], $record);
            }
        }
    }
    ...
    protected static function get_table_user_map(\stdClass $user): array {
        $tables = [
            'auth_cnoauth_prevlogin' => ['userid' => $user->id],
            'auth_cnoauth_token' => ['userid' => $user->id],
        ];
        return $tables;
    }
    ...
    private static function delete_user_data(int $userid) {
        global $DB;
        $DB->delete_records('auth_cnoauth_prevlogin', ['userid' => $userid]);
        $DB->delete_records('auth_cnoauth_token', ['userid' => $userid]);
    }
