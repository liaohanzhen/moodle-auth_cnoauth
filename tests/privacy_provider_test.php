...
use \auth_cnoauth\privacy\provider;

/**
 * Privacy test for auth_cnoauth
 *
 * @group auth_cnoauth
 * @group auth_cnoauth_privacy
 * @group office365
 * @group office365_privacy
 */
class auth_cnoauth_privacy_testcase extends \core_privacy\tests\provider_testcase {
    /**
     * Tests set up.
     */
    ...
    public function test_export_user_data() {
        ...
        $approvedlist = new core_privacy\local\request\approved_contextlist($user, 'auth_cnoauth', [$usercontext->id]);
        provider::export_user_data($approvedlist);
        // Token.
        $data = $writer->get_data([
            get_string('privacy:metadata:auth_cnoauth', 'auth_cnoauth'),
            get_string('privacy:metadata:auth_cnoauth_token', 'auth_cnoauth')
        ]);
        ...
        $data = $writer->get_data([
            get_string('privacy:metadata:auth_cnoauth', 'auth_cnoauth'),
            get_string('privacy:metadata:auth_cnoauth_prevlogin', 'auth_cnoauth')
        ]);
    ...
    public function test_delete_data_for_all_users_in_context() {
        ...
        $this->assertCount(2, $DB->get_records('auth_cnoauth_token', []));
        $this->assertCount(2, $DB->get_records('auth_cnoauth_prevlogin', []));

        // Delete everything for the first user context.
        provider::delete_data_for_all_users_in_context($user1context);

        $this->assertCount(0, $DB->get_records('auth_cnoauth_token', ['userid' => $user1->id]));
        $this->assertCount(0, $DB->get_records('auth_cnoauth_prevlogin', ['userid' => $user1->id]));

        // Get all accounts. There should be one.
        $this->assertCount(1, $DB->get_records('auth_cnoauth_token', []));
        $this->assertCount(1, $DB->get_records('auth_cnoauth_prevlogin', []));
    }
    ...
    public function test_delete_data_for_user() {
        ...
        $approvedlist = new \core_privacy\local\request\approved_contextlist($user1, 'auth_cnoauth', [$user1context->id]);
    ...
    static private function create_token(int $userid): \stdClass {
        global $DB;
        $record = new stdClass();
        $record->cnoauthuniqid = "user@example.com";
        $record->username = "user@example.com";
        $record->userid = $userid;
        $record->cnoauthusername = "user@example.com";
        $record->scope = "All";
        ...
        $record->id = $DB->insert_record('auth_cnoauth_token', $record);
        return $record;
    }
    ...
    static private function create_prevlogin(int $userid): \stdClass {
        ...
        $record->id = $DB->insert_record('auth_cnoauth_prevlogin', $record);
        return $record;
    }
